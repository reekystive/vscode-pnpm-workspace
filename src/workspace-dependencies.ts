...
export async function getWorkspaceDependencyNames(
  ...
  try {
    ...
    for (const [name, version] of Object.entries(allDeps)) {
      ...
      if (typeof version === 'string' && version.startsWith('workspace:')) {
        ...
        if (depPackage) {
          log(`Added workspace dependency name: ${name}`);
  ...
  log(`Final workspace dependency names for ${packageName}: [${dependencyNames.join(', ')}]`);
 ...
 */
export async function getWorkspaceDependencies(packageName: string, packages: WorkspacePackage[]): Promise<string[]> {
  log('=================================');
